// Written by Denise Verbakel

/*
 * Include the files needed in this program.
*/

#include <stdio.h>
#include <inttypes.h>

/*
 * Specify the S-box of Skinny128.
*/

const uint8_t sbox[] = {
	
};

/*
 * This function is used to convert decimal numbers to binary numbers.
*/

void bin(uint8_t n, FILE *fp)
{
    uint8_t i;
    for (i = 1 << 7; i > 0; i = i / 2)
        (n & i) ? fprintf(fp, "1") : fprintf(fp, "0");
}
 

int main()
{
    /*
     * We create a file called skinny128.in, write the input for the Espresso algorithm to this file and 
     * then close this file again.
    */

    FILE *fp;
    fp = fopen("skinny128.in", "w+");

    fprintf(fp, ".i 8               			# .i specifies the number of inputs\n");
    fprintf(fp, ".o 8               			# .o specifies the number of outputs\n");
    fprintf(fp, ".ilb X0 X1 X2 X3 X4 X5 X6 X7      	# .ilb specifies the names of the inputs in order\n");
    fprintf(fp, ".ob Y0 Y1 Y2 Y3 Y4 Y5 Y6 Y7 	  	# .ob specifies the names of the outputs in order\n");

    for (int i = 0; i < 256; i++) {
	bin(i, fp);
	fprintf(fp, " ");
    	bin(sbox[i], fp);
	fprintf(fp, "\n");
    }

    fprintf(fp, ".e       				# Signifies the end of the file");

    fclose(fp);
}
