...
Table_fixed *
convolute_tables_fixed(Table_fixed *t, uint8_t w, int lower_bound, int upper_bound) 
{
	...
	for (int j = i-1; j >= 0; j--) {
		// r = r*r
		Table_fixed *tmp  = convolute_fixed(r, r, lower_bound, upper_bound);
		table_fixed_destroy(r);
		r = tmp;
		if ((w & (1 << j)) != 0) {
			// r = r*t
			tmp = convolute_fixed(r, t, lower_bound, upper_bound);
			table_fixed_destroy(r);
			r = tmp;
		}	
	}
...
Table *
convolute_tables(Table *t, uint8_t w, int lower_bound, int upper_bound, void (*f[3])(mpz_t, const mpz_t, const mpz_t)) 
{
	// We search for the most significant bit
	int i = 0;
	for (int j = 7; j >= 0; j--) {
	...
	for (int j = i-1; j >= 0; j--) {
		// r = r*r
		Table *tmp  = convolute(r, r, lower_bound, upper_bound, f);
		table_destroy(r);
		r = tmp;
		if ((w & (1 << j)) != 0) {
			// r = r*t
			tmp = convolute(r, t, lower_bound, upper_bound, f);
			table_destroy(r);
			r = tmp;
		}	
	}
...
Table_fixed *
convolute_fixed(Table_fixed *t1, Table_fixed *t2, int lower_bound, int upper_bound)
{
...
Table *
convolute(Table *t1, Table *t2, int lower_bound, int upper_bound, void (*f[3])(mpz_t, const mpz_t, const mpz_t))
{
