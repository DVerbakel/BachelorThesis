...
void (*trail_weight_combiners[3])(mpz_t, const mpz_t, const mpz_t) = {&mpz_mul, &mpz_mul, &mpz_mul};
...
uint32_t
box_weight32(uint32_t a, int number_of_boxes, int box_width) {
	uint32_t weight = 0;
...
uint64_t
box_weight64(uint64_t a, int number_of_boxes, int box_width) {
	uint64_t weight = 0;
	for (int i = 0; i < number_of_boxes; i++) {
		if ((a & ((1 << box_width)-1)) != 0) {
			weight++;
