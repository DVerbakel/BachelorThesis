...
triples = [('../data/aes-linear-trail-superbox.txt', 'D-', 'r', 'Rijndael'),
           ('../data/saturnin-linear-trail-superbox.txt', 's-', 'b', 'Saturnin'),
           ('../data/skinny64-linear-trail-superbox.txt', '<-', '#FF69B4', 'Skinny64'),
           ('../data/spongent-linear-trail-superbox.txt', 'X-', 'y',  'Spongent')]
...
# full linear trail weights and then plot the cumulative linear trail weight histograms of the superboxes.
...
plt.xlim(0, 50)
plt.ylim(0, 140)
plt.xlabel('Linear Trail Weight Superbox', fontsize=16)
plt.ylabel('Log2(Number of Trails)', fontsize=16)

# Set the legend and grid of the plot and save this to the file linear_trail_weights_superbox.pdf

plt.legend(loc='upper left', fontsize = 'small')
plt.grid(b=True, which='major')
plt.savefig('results/linear_trail_weights_superbox.pdf', bbox_inches='tight')
