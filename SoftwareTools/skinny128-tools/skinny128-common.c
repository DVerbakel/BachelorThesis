// Written by Denise Verbakel

/*
 * Include the file needed in this program.
*/

#include "skinny128-common.h"

/*
 * S-box and inverse S-box of Skinny128 in hexadecimal notation.
*/

const uint8_t sbox[] = {
	
};

const uint8_t inv_sbox[] = {
	
};

/*
 * The MixColumns (MC) layer of Skinny128. 
 * All nibble columns of the ciphers internal state are multiplied by the following matrix:
 *	1  0  1  1
 *	1  0  0  0
 *	0  1  1  0
 *	1  0  1  0
*/

uint32_t 
mix_columns(uint32_t a)
{	
	uint32_t first = (a >> 24) & 0xff;
	uint32_t second = (a >> 16) & 0xff;
	uint32_t third = (a >> 8) & 0xff;
	uint32_t fourth = a & 0xff;

	uint32_t new_second = first;
	uint32_t new_third = second ^ third;
	uint32_t new_fourth = first ^ third;
	uint32_t new_first = new_fourth ^ fourth;

	return (new_first << 24) | (new_second << 16) | (new_third << 8) | new_fourth;
}

/*
 * The transposed MixColumns (MC) layer of Skinny128. 
 * All nibble columns of the ciphers internal state are multiplied by the following matrix:
 *	1  1  0  1
 *	0  0  1  0
 *	1  0  1  1
 *	1  0  0  0
*/

uint32_t 
mix_columns_transpose(uint32_t a)
{
	uint32_t first = (a >> 24) & 0xff;
	uint32_t second = (a >> 16) & 0xff;
	uint32_t third = (a >> 8) & 0xff;
	uint32_t fourth = a & 0xff;

	uint32_t intermediate = first ^ fourth;

	uint32_t new_first = intermediate ^ second;
	uint32_t new_second = third;
	uint32_t new_third = intermediate ^ third;
	uint32_t new_fourth = first;

	return (new_first << 24) | (new_second << 16) | (new_third << 8) | new_fourth;
}

/*
 * The SubCells (SC) layer of Skinny128. 
 * The 8-bit S-box as defined above is applied to each cell of the ciphers internal state.
*/

uint32_t
sub_cells(uint32_t x)
{
   	uint32_t a = x & 0xff;
 	uint32_t b = (x >> 8) & 0xff;
 	uint32_t c = (x >> 16) & 0xff;
 	uint32_t d = (x >> 24) & 0xff;

	uint32_t s_a = sbox[a];		// S(a)
	uint32_t s_b = sbox[b];		// S(b)
	uint32_t s_c = sbox[c];		// S(c)
	uint32_t s_d = sbox[d];		// S(d)

	return (s_d << 24) | (s_c << 16) | (s_b << 8) | s_a;
}
