...
main(void)
{
	...
	*/
	 ...
	 * combinations for the pair (input,output) in a table. Lastly, we save the results to the file skinny128-differential-box-superbox-2D.txt.
	*/

	for (uint64_t a = 0; a <= UINT32_MAX; a++) {
		mpz_set_ui(row, box_weight64(a, NUMBER_OF_BOXES_IN_SUPERBOX, BOX_WIDTH));
		mpz_set_ui(col, box_weight64(mix_columns(a), NUMBER_OF_BOXES_IN_SUPERBOX, BOX_WIDTH));
	...
	table_print("data/skinny128-differential-box-superbox-2D.txt", box_weights_superbox);
