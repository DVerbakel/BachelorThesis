...
main(void)
{
	...
	*/
	...
	/*
	 * We get the hamming weight from the table defined in skinny128-lookup-table.c. We compute the hamming weight of the input found in the
	 ...
	 * column. We merge the entries with the same combinations for the pair (input,output) in a table. Lastly, we save the results to the 
         * file skinny128-differential-hamming-weight-2D.txt.
	*/	

	for (int i = 0; i < 256; i++) {
		mpz_set_ui(row, lookup_table_differential_hamming_weight[i][0]);
		mpz_set_ui(col, lookup_table_differential_hamming_weight[i][1]);
		table_insert_and_merge(hamming_weights, row, col, val, &mpz_add);
	}
	
	mpz_clears(row, col, val, NULL);
	
	// Data: (hamming weight input, hamming weight output, number of differences)
	table_print("data/skinny128-differential-hamming-weight-2D.txt", hamming_weights);
