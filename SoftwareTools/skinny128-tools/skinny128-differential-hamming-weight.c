...
main(void)
{
	...
	*/
	 ...
	 * We get the hamming weight from the table defined in skinny128-lookup-table.c. We compute for difference a: 
         ...
         * results to the file skinny128-differential-hamming-weight.txt.
	*/
	...
	table_print("data/skinny128-differential-hamming-weight.txt", hamming_weights);
	
	/*
	 * Convolute the above results of one column in order to compute the histogram of the full state(s). We initialize the
	 * table structure and save the results to the files skinny128-differential-hamming-weight-full.txt,
	 * skinny128-artificial-skinny256-differential-hamming-weight-full.txt and
	...
	*/
	...
	// Data: (hamming weight, not used, number of differences)
	table_print("data/skinny128-differential-hamming-weight-full.txt", hamming_weights_full);
	table_print("data/skinny128-artificial-skinny256-differential-hamming-weight-full.txt", hamming_weights_full_256);
	table_print("data/skinny128-artificial-skinny384-differential-hamming-weight-full.txt", hamming_weights_full_384);
