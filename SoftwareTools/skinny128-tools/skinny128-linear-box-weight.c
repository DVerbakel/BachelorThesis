...
main(void)
{
	...
	*/
	 ...
	 * We get the box weight using the function box_weight64 defined in global-common.c. We compute for difference a: 
         * w := bw(a) + bw(MixColumnsTranspose(a)). Then we increment the entry indexed by w in the histogram by 1. Lastly, we save the results to the
         * file skinny128-linear-box-superbox.txt.
	*/

	for (uint64_t a = 0; a <= UINT32_MAX; a++) {
		mpz_set_ui(row, box_weight64(a, NUMBER_OF_BOXES_IN_SUPERBOX, BOX_WIDTH) + box_weight64(mix_columns_transpose(a), NUMBER_OF_BOXES_IN_SUPERBOX, BOX_WIDTH));
		table_insert_and_merge(box_weights_superbox, row, col, val, &mpz_add);
	}	

	mpz_clears(row, col, val, NULL);

	// Data: (box weight, not used, number of masks) 
	table_print("data/skinny128-linear-box-superbox.txt", box_weights_superbox);
	 ...
	 * structure and save the results to the files skinny128-linear-box-full.txt, skinny128-artificial-skinny256-linear-box-full.txt
	 * and skinny128-artificial-skinny384-linear-box-full.txt.
	*/
	...
	// Data: (box weight, not used, number of masks) 
	table_print("data/skinny128-linear-box-full.txt", box_weights_full);
	table_print("data/skinny128-artificial-skinny256-linear-box-full.txt", box_weights_full_256);
	table_print("data/skinny128-artificial-skinny384-linear-box-full.txt", box_weights_full_384);
