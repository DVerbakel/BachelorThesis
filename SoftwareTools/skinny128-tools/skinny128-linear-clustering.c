...
{
	uint16_t pattern = 0;
	uint32_t b = mix_columns_transpose(a);
...
main(void)
{
	...
	*/
	 ...
	 * box_partition_superbox. Lastly, we save the results to the file skinny128-linear-clustering-superbox.txt.
	*/
	...
	table_print("data/skinny128-linear-clustering-superbox.txt", box_partition_superbox);
	 ...
	 * structure and save the results to the file skinny128-linear-clustering-full.txt.
	*/

	Table *box_partition_full = convolute_tables(box_partition_superbox, NUMBER_OF_BOXES_IN_SUPERBOX, 0, 0, differential_combiners);
	
	// Data: (weight, cardinality of cluster class, number of cluster classes)
	table_print("data/skinny128-linear-clustering-full.txt", box_partition_full);	
