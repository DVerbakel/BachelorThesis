...
{	
	...
	*/
	 ...
	 * After having done this, we save the results to the file skinny128-sbox-twice.txt
	*/
	...
	for (uint64_t v = 0; v < SIZE; v++) {
		...
		for (uint64_t x = 0; x < SIZE; x++) {
			...
			uint32_t y_two_round = sub_cells(y_one_round);					// S(S(x))
	...
	table_fixed_print("data/skinny128-sbox-twice.txt", superbox_counts);
