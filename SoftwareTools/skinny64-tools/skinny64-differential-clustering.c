...
main(void)
{
	...
	*/
	 ...
	 * box_partition_superbox. Lastly, we save the results to the file skinny64-differential-clustering-superbox.txt.
	*/
	...
	table_print("data/skinny64-differential-clustering-superbox.txt", box_partition_superbox);

	/*
	 * Convolute the above results of the superboxes in order to compute the histogram of the full state. We initialize 
	 * the table structure and save the results to the file skinny64-differential-clustering-full.txt.
	*/

	Table *box_partition_full = convolute_tables(box_partition_superbox, NUMBER_OF_BOXES_IN_SUPERBOX, 0, 0, differential_combiners);
	
	// Data: (weight, cardinality of cluster class, number of cluster classes)
	table_print("data/skinny64-differential-clustering-full.txt", box_partition_full);	
