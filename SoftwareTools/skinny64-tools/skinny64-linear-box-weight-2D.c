...
main(void)
{
	...
	*/
	 ...
	 * same combinations for the pair (input,output) in a table. Lastly, we save the results to the file skinny64-linear-box-superbox-2D.txt.
	*/

	for (uint32_t a = 0; a <= UINT16_MAX; a++) {
		mpz_set_ui(row, box_weight32(a, NUMBER_OF_BOXES_IN_SUPERBOX, BOX_WIDTH));
		mpz_set_ui(col, box_weight32(mix_columns_transpose(a), NUMBER_OF_BOXES_IN_SUPERBOX, BOX_WIDTH));
	...
	table_print("data/skinny64-linear-box-superbox-2D.txt", box_weights_superbox);
