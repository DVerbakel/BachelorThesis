...
main(void)
{
	...
	*/
	 ...
	 * We get the box weight using the function box_weight32 defined in global-common.c. We compute for difference a: 
         ...
         * file skinny64-linear-box-superbox.txt.
	*/

	for (uint32_t a = 0; a <= UINT16_MAX; a++) {
		mpz_set_ui(row, box_weight32(a, NUMBER_OF_BOXES_IN_SUPERBOX, BOX_WIDTH) + box_weight32(mix_columns_transpose(a), NUMBER_OF_BOXES_IN_SUPERBOX, BOX_WIDTH));
	...
	table_print("data/skinny64-linear-box-superbox.txt", box_weights_superbox);
	 ...
	 * structure and save the results to the files skinny64-linear-box-full.txt, skinny64-artificial-skinny256-linear-box-full.txt
	 * and skinny64-artificial-skinny384-linear-box-full.txt.
	*/
	...
	// Data: (box weight, not used, number of masks) 
	table_print("data/skinny64-linear-box-full.txt", box_weights_full);	
	table_print("data/skinny64-artificial-skinny256-linear-box-full.txt", box_weights_full_256);
	table_print("data/skinny64-artificial-skinny384-linear-box-full.txt", box_weights_full_384);
